import {
	getAllBlogs, createCard, getBlogCategoryPages, createTag, getAllArticles
...
function updateFiltersCount(count) {
	...
	const mobileFiltersCount = document.querySelector('.thought-leadership-home .filters > .filters-header > h4');
...
function clearFilters() {
	...
	const selectedFilters = document.querySelector('.thought-leadership-home .selected-filters');
...
if (contentTypes.size) {
  ...
  filtersMain.append(contentTypeElement);
	...
	const blogHomeEl = createTag('div', { class: 'thought-leadership-home-link' });
	...
	if (/(^\/blog$)/.test(window.location.pathname)) {
		...
		blogHomeLink.innerHTML += '<h4>Thought leadership</h4>';
	} else {
		blogHomeLink.innerHTML += 'Thought leadership';
	}
